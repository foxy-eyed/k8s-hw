output "service_account_name" {
  ...
  value       = yandex_iam_service_account.k8s-sa.name
}
...
output "container_registry_name" {
  description = "Имя контейнерного реестра"
  value       = yandex_container_registry.rails-app-cr.name
}

output "container_registry_id" {
  description = "ID контейнерного реестра"
  value       = yandex_container_registry.rails-app-cr.id
}
